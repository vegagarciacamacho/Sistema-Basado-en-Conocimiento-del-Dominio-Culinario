...
class TestMotor(unittest.TestCase):
    def setUp(self):
        ...
        if self.use_full:
            ...
            archivo_bc = (
                Path(__file__).parent.parent / "kb" / "bc.txt"
            )  # Construye la ruta completa al archivo bc.txt en la carpeta "kb"
            self.hechos, self.reglas = cargar(
                archivo_bc
            )  # Llama a la función cargar para cargar hechos y reglas desde el archivo bc.txt
        else:
            # crear un archivo temporal dentro de tests/
            temp_path = (
                Path(__file__).parent / "temp_bc_small.txt"
            )  # Construye la ruta del archivo temporal en el directorio de tests
            temp_path.write_text(
                SMALL_KB, encoding="utf-8"
            )  # Escribe la base de datos pequeña en el archivo temporal
            self.hechos, self.reglas = cargar(
                temp_path
            )  # Carga los hechos y reglas desde este archivo temporal
    ...
    def test_consultas_y_modificaciones(self):
        ...
        out = _capture(ejecutar_consulta, "pepino color X?", self.hechos, [])
        self.assertTrue(
            ("No se encontraron coincidencias" in out) or ("No, no está" in out)
        )
    ...
    def test_ingredientes_y_recetas(self):
        if not self.use_full:
            self.skipTest(
                "Test de ingredientes y recetas solo se ejecuta con la base de datos completa."
            )
        ...
        out = _capture(
            _razona_cli,
            "razona si bica_blanca_laza receta_totalmente_disponible true?",
            self.hechos,
            self.reglas,
        )
        self.assertIn(
            "Sí, se puede deducir: bica_blanca_laza receta_totalmente_disponible true con certeza 0.95",
            out,
        )
        ...
        out = _capture(
            ejecutar_consulta,
            "ensalada_garbanzos receta_muy_facil X?",
            self.hechos,
            hechos_deducidos,
        )
        ...
        out = _capture(
            ejecutar_consulta,
            "bica_blanca_laza no_apta_lactosa X?",
            self.hechos,
            hechos_deducidos,
        )
    ...
    def test_maridajes(self):
        if not self.use_full:
            self.skipTest(
                "Test de maridajes solo se ejecuta con la base de datos completa."
            )
        ...
        out = _capture(
            ejecutar_consulta, "cerveza_suave marida ensalada?", self.hechos, []
        )
        ...
        out = _capture(
            _razona_cli,
            "razona si vino_blanco acompaña_receta quiche_de_salmon?",
            self.hechos,
            self.reglas,
        )
        self.assertTrue(
            "Sí, se puede deducir: vino_blanco acompaña_receta quiche_de_salmon con certeza 0.85"
            in out
        )
